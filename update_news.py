#!/usr/bin/env python3
"""APEX TCG Intel daily updater. News/dates injected here; FX in fx_helper.py."""
import re, sys, time
from datetime import datetime, timezone, timedelta
from html import escape
import requests
from bs4 import BeautifulSoup
from fx_helper import fetch_fx, inject_fx
...
def main():
    ...
    poke,op=pkmn(),onepc()
    fl=[i for i in poke+op if newdate(i)]
    up,jp,dbg=fetch_fx(fetch,H,TIMEOUT)
    print(f"Pokemon:{len(poke)} OnePiece:{len(op)} flagged:{len(fl)} FX:{up}/{jp} ({dbg})")
    h=inject(h,"pokemon-news-live","pokeUpdatedNote",poke)
    h=inject(h,"onepiece-news-live","opUpdatedNote",op)
    h=inj_auto(h,fl)
    h=inject_fx(h,up,jp,dbg)
